#' Pipeline.SpeciesNetworkDistances.ScaleAllBranches: function to measure phylogenetic distances across a range of branch length scaling between two species tree models
#'
...
#####################################################
# Pipeline.SpeciesNetworkDistances.ScaleAllBranches #
#####################################################
Pipeline.SpeciesNetworkDistances.ScaleAllBranches <- function(vector.ScaleFactor, string.PathParentDir, string.PathHybridCoal, handle.SpeciesNetwork.Model1, handle.SpeciesNetwork.Model2){
  
  ##################################################
  # Create directory for analysis with hybrid-coal #
  ##################################################
  string.ChildDir0 = paste(string.PathParentDir, '/Pipeline_PSTD_NETWORK_ScaleAllBranches_', Sys.Date(), sep = "")
  unlink(string.ChildDir0, recursive = T)
  dir.create(string.ChildDir0, showWarnings = T, recursive = T)
  matrix.DistanceResults <- matrix(nrow = length(vector.ScaleFactor), ncol = 3)
  rownames(matrix.DistanceResults) <- vector.ScaleFactor
  colnames(matrix.DistanceResults) <- c("Hellinger", "Kullback_Leibler", "Jensen_Shannon")
  ...
  for (i in 1:length(vector.ScaleFactor)){
    ...
    ################################################
    handle.SpeciesNetwork.Model2.i <- handle.SpeciesNetwork.Model2
    handle.SpeciesNetwork.Model2.i$edge.length <- handle.SpeciesNetwork.Model2.i$edge.length*numeric.Scale.i
    
    ############################
    # Conduct PSTD analyses... #
    ############################
    vector.Probabilistic.SpeciesTree.Distances <- Compute.Probabilistic.SpeciesTree.Distances(handle.SpeciesTree.Model1 = handle.SpeciesNetwork.Model1, 
                                                                                              handle.SpeciesTree.Model2 = handle.SpeciesNetwork.Model2.i, 
    ...
    ############################
    matrix.DistanceResults[i,] <- c(numeric.Hellinger, numeric.Kullback_Leibler, numeric.Jensen_Shannon)
